...
import { ArrowLeft, Car, Database, RefreshCw, FileText } from "lucide-react";
import Link from "next/link";

export default function MovilesDiazDetail() {
    return (
                    ...
                    <div className="grid lg:grid-cols-2 gap-16 items-center mb-32">
                        ...
                        >
                            <span className="inline-block px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-bold uppercase tracking-widest mb-6">
                                Plataforma de Logística & Flota
                            </span>
                            <h1 className="text-6xl md:text-7xl font-black tracking-tighter mb-8 leading-tight">
                                Móviles Díaz <br />
                                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">Gestión de Transporte</span>
                            </h1>
                            <p className="text-xl text-slate-300 font-medium leading-relaxed mb-8">
                                Sistema a la medida para la administración de flotas de pasajeros, control de viajes, cobros automatizados, cálculo inteligente de comisiones y auditoría diaria de operaciones.
                            </p>
                            <div className="flex flex-wrap gap-3">
                                {["React", "Vite", "Offline Sync", "Azure", "Tailwind"].map(tag => (
                        ...
                        >
                            ...
                            <img
                                src="/projects/moviles-diaz.png"
                                alt="Móviles Díaz Interface"
                                className="rounded-[2.5rem] shadow-2xl border border-white/10 w-full"
                            />
                        </motion.div>
                    </div>

                    <h2 className="text-4xl font-black mb-16 tracking-tight text-center">Módulos Estratégicos</h2>
                    <div className="grid md:grid-cols-3 gap-8 mb-32">
                        <ModuleCard
                            icon={<RefreshCw className="text-indigo-400" />}
                            title="Sincronización Offline-First"
                            desc="Cola de transacciones local inteligente (SyncAction) que almacena los cambios en caché y los sincroniza automáticamente cuando se recupera la conexión."
                        />
                        <ModuleCard
                            icon={<Database className="text-cyan-400" />}
                            title="Auditoría e Informes"
                            desc="Generación de reportes de facturación de clientes, liquidaciones de pago de conductores y resúmenes periódicos basados en cierres automáticos."
                        />
                        <ModuleCard
                            icon={<FileText className="text-purple-400" />}
                            title="Control Financiero"
                            desc="Cálculo dinámico de reembolsos (estacionamientos, TAG, peajes) y cálculo automatizado de pagos finales a conductores según porcentajes de comisión configurables."
                        />
                    </div>
                </div>
            </main>
