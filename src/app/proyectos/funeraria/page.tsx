...
import { ArrowLeft, Layers, Lock, Database } from "lucide-react";
                    ...
                    <div className="grid lg:grid-cols-2 gap-16 items-center mb-32">
                        ...
                        >
                            <span className="inline-block px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-bold uppercase tracking-widest mb-6">
                                Gestión Documental y Operatividad
                            </span>
                            <h1 className="text-6xl md:text-7xl font-black tracking-tighter mb-8 leading-tight">
                                Sistema <br />
                                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">Funerario</span>
                            </h1>
                            <p className="text-xl text-slate-300 font-medium leading-relaxed mb-8">
                                Herramienta integral enfocada en la discreción operativa, trazabilidad sin fallos y agilización de trámites a través de una UI limpia y respetuosa.
                            </p>
                            <div className="flex flex-wrap gap-3">
                                {["Next.js", "Prisma", "PostgreSQL", "NextAuth"].map(tag => (
                        ...
                        >
                            ...
                            <img
                                src="/projects/funeraria.png"
                                alt="Dashboard Sistema Funerario"
                                className="rounded-[2.5rem] shadow-2xl border border-white/10 w-full"
                            />
                        </motion.div>
                    </div>

                    <h2 className="text-4xl font-black mb-16 tracking-tight text-center">Fuerza Operativa</h2>
                    <div className="grid md:grid-cols-3 gap-8 mb-32">
                        <ModuleCard
                            icon={<Layers className="text-indigo-400" />}
                            title="Gestión de Casos"
                            desc="Organización impecable del ciclo completo de los procesos de servicio y documentación."
                        />
                        <ModuleCard
                            icon={<Database className="text-cyan-400" />}
                            title="Persistencia Sólida"
                            desc="Cimientos en PostgreSQL a través de Prisma garantizando redundancia y cero pérdida de historial."
                        />
                        <ModuleCard
                            icon={<Lock className="text-purple-400" />}
                            title="Alta Privacidad"
                            desc="Restricción minuciosa de niveles corporativos amparada en NextAuth Core Authentication."
                        />
                    </div>
                </div>
            </main>
